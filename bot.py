...
from config import (
    ...
    ROLA_KMP, ROLA_SPD, ROLA_JRG, ROLA_RSPR, ROLA_GDDKIA,
    KANAL_LOGI_PODAN
)
...
# =====================================================
# MAPA FRAKCJA -> ROLA (latwo rozszerzalna)
# =====================================================
FRAKCJE_ROLE = {
    "kmp": ROLA_KMP,
    "spd": ROLA_SPD,
...
FRAKCJE_NAZWY = {
    ...
    "spd": "Sluzby Pomocy Drogowej",
    "jrg": "Jednostka Ratowniczo-Gasnicza",
    "rspr": "Rejonowa Stacja Pogotowia Ratunkowego",
    "gddkia": "GDDKiA",
}
...
def webhook_handler():
    ...
    try:
        ...
        kanaly = {
            ...
            "gddkia": KANAL_PODANIA_GDDKIA if 'KANAL_PODANIA_GDDKIA' in globals() else None,
        }
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO podania (discord_id, discord_username, frakcja, frakcja_nazwa, odpowiedzi, timestamp, message_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (discord_id, discord_username, frakcja, frakcja_nazwa, json.dumps(odpowiedzi), timestamp, message_id)
        )
        podanie_id = cursor.lastrowid
        
        # Zapisz tez do podania_status
        cursor.execute(
            "INSERT INTO podania_status (podanie_id, discord_id, frakcja, message_id) VALUES (?, ?, ?, ?)",
            (podanie_id, discord_id, frakcja, message_id)
        )
...
async def odtworz_panel_weryfikacji():
    ...
    if usuniete > 0:
        print(f"[WERYFIKACJA] Usunieto {usuniete} starych paneli")
...
class WeryfikacjaModal(discord.ui.Modal, title="Weryfikacja - Krok 1"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO kody_weryfikacyjne (discord_id, kod, roblox_id, roblox_username) VALUES (?, ?, ?, ?)",
...
# =====================================================
# ZARZADZANIE PODANIAMI (AKCEPTACJA/ODRZUCENIE)
# =====================================================
@bot.tree.command(name="panel-podan", description="Wyslij panel do zarzadzania podaniami")
@commands.has_permissions(administrator=True)
async def panel_podan(interaction: discord.Interaction):
    embed = discord.Embed(
        title="Panel Zarzadzania Podaniami",
        description="Kliknij przycisk, aby zobaczyc oczekujace podania.",
        color=0x00D4AA
    )
    await interaction.response.send_message(embed=embed, view=PanelPodanView(), ephemeral=True)

class PanelPodanView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Pokaz oczekujace", style=discord.ButtonStyle.primary, emoji="📋")
    async def pokaz(self, interaction: discord.Interaction, button: discord.ui.Button):
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT ps.id, ps.discord_id, ps.frakcja, p.discord_username, p.odpowiedzi, p.timestamp, ps.message_id
            FROM podania_status ps
            JOIN podania p ON ps.podanie_id = p.id
            WHERE ps.status = 'oczekujace'
            ORDER BY ps.id DESC
            LIMIT 10
        """)
        podania = cursor.fetchall()
        conn.close()

        if not podania:
            await interaction.response.send_message("Brak oczekujacych podan.", ephemeral=True)
            return

        ps_id, dc_id, frak, username, odpowiedzi, timestamp, msg_id = podania[0]
        frakcja_nazwa = FRAKCJE_NAZWY.get(frak, frak.upper())
        
        embed = discord.Embed(
            title=f"Podanie #{ps_id} - {frakcja_nazwa}",
            description=f"**Od:** {username}\n**ID:** `{dc_id}`\n**Data:** {timestamp[:19].replace('T', ' ')}",
            color=0xFFA500
        )
        
        await interaction.response.send_message(
            embed=embed,
            view=AkceptujOdrzucView(ps_id, dc_id, frak),
            ephemeral=True
        )

class AkceptujOdrzucView(discord.ui.View):
    def __init__(self, podanie_status_id, discord_id, frakcja):
        super().__init__(timeout=None)
        self.podanie_status_id = podanie_status_id
    ...
    async def _przetworz(self, interaction, status, wynik, nadaj_role):
        ...
        # Odpowiedz recenzentowi
        await interaction.response.send_message(
            f"Podanie #{self.podanie_status_id} **{status}**.\n"
            f"DM: {dm_status}\n"
            f"Rola: {rola_nadana}",
            ephemeral=True
        )

        # Log do kanalu (opcjonalnie)
        if KANAL_LOGI_PODAN:
...
@app_commands.describe(
    numer_postaci="Postac",
    imie_nazwisko="Imie i nazwisko",
    data_urodzenia="DD.MM.RRRR",
    obywatelstwo="Obywatelstwo"
)
@app_commands.choices(numer_postaci=[
    app_commands.Choice(name="Postac 1", value=1),
    app_commands.Choice(name="Postac 2", value=2)
])
async def dowod(
    ...
    if not dane:
        await interaction.response.send_message("Zweryfikuj sie przez Roblox", ephemeral=True)
        return
    ...
    if cursor.fetchone():
        conn.close()
        await interaction.response.send_message(
            f"Masz juz dowod Postaci {numer_postaci.value}",
            ephemeral=True
        )
        return
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds={roblox_id}&size=420x420&format=Png"
        ) as resp:
            data = await resp.json()
            if data.get("data"):
    ...
    cursor.execute(
        """INSERT INTO dowody
        (discord_id, numer_postaci, imie_nazwisko, data_urodzenia, obywatelstwo, ssn, roblox_username, roblox_avatar)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
    ...
    await interaction.response.send_message(
        f"Dowod Postaci {numer_postaci.value} wydany",
        ephemeral=True
    )
...
async def wystaw_wyrok(interaction: discord.Interaction, osoba: discord.Member, kara_wiezienia: str, kwota: str, zarzuty: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO wyroki (osoba_id, osoba_nick, funkcjonariusz, kara_wiezienia, kwota, zarzuty, data) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
async def wystaw_mandat(interaction: discord.Interaction, osoba: discord.Member, kwota: str, zarzuty: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO mandaty (osoba_id, osoba_nick, funkcjonariusz, kwota, zarzuty, data) VALUES (?, ?, ?, ?, ?, ?)",
...
async def rejestruj_pojazd(interaction: discord.Interaction, nr: str, marka: str, wlasciciel: discord.Member):
    ...
    if cursor.fetchone():
        conn.close()
        await interaction.response.send_message("Numer zajety", ephemeral=True)
        return

    cursor.execute(
        "INSERT INTO rejestracje (nr_rejestracyjny, marka_model, wlasciciel_id, wlasciciel_nick) VALUES (?, ?, ?, ?)",
...
async def reset_weryfikacji(interaction: discord.Interaction, osoba: discord.Member):
    ...
    if not wynik:
        conn.close()
        await interaction.response.send_message(f"{osoba.mention} niezweryfikowany", ephemeral=True)
        return
    ...
    await interaction.response.send_message(
        f"Weryfikacja {osoba.mention} (`{roblox_username}`) zresetowana",
        ephemeral=True
    )
