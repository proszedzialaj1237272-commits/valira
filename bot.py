...
# =====================================================
# PRZYCISKI POD EMBEDEM PODANIA
# =====================================================
class PodanieReviewView(discord.ui.View):
...
def webhook_handler():
    ...
    try:
        ...
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("UPDATE podania SET message_id = ? WHERE id = ?", (message_id, podanie_id))
        cursor.execute("UPDATE podania_status SET message_id = ? WHERE id = ?", (message_id, ps_id))
        conn.commit()
        conn.close()
